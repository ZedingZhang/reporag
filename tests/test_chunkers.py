...
class TestMarkdownChunker:
    def test_splits_by_heading_level(self, sample_markdown: str) -> None:
        chunks = chunk_markdown(
            sample_markdown, "README.md", "abc123",
    ...
    def test_empty_text_produces_no_chunks(self) -> None:
        chunks = chunk_markdown(
            "", "empty.md", "abc123",
            "https://github.com/o/r/blob/abc123/empty.md",
        )
        assert len(chunks) == 0
...
class TestPythonChunker:
    def test_extracts_functions(self, sample_python_code: str) -> None:
        chunks = chunk_code_file(
            sample_python_code, "test.py", "abc123",
            "https://github.com/o/r/blob/abc123/test.py",
        )
        func_names = [
            c.symbol_name
            for c in chunks
        ...
        assert "standalone_async" in func_names
    ...
    def test_extracts_classes(self, sample_python_code: str) -> None:
        ...
        class_chunks = [
            c for c in chunks if c.metadata.get("symbol_type") == "class"
        ]
    ...
    def test_methods_have_parent_class_metadata(
    ...
    def test_syntax_error_fallback(self) -> None:
        ...
        chunks = chunk_code_file(
            invalid_py, "broken.py", "abc123",
            "https://github.com/o/r/blob/abc123/broken.py",
        )
        assert len(chunks) > 0
...
class TestJSChunker:
    def test_extracts_js_functions(self, sample_js_code: str) -> None:
        chunks = chunk_code_file(
            sample_js_code, "test.js", "abc123",
            "https://github.com/o/r/blob/abc123/test.js",
        )
...
class TestTextBlockChunker:
    def test_short_text_single_chunk(self) -> None:
        ...
        chunks = chunk_text_blocks(
            text, None, "https://github.com/o/r/issues/1", "issue_comment",
        )
        assert len(chunks) == 1
    ...
    def test_long_text_splits(self) -> None:
        text = "Paragraph. " * 2000
        chunks = chunk_text_blocks(
            text, None, "https://example.com", "pr_description",
        )
        assert len(chunks) > 1
        for c in chunks:
